![Um Guia Aberto](figures/signpost-horiz1-1600.jpg)

O Guia Aberto para Amazon Web Services
======================================

[![Slack Chat](https://img.shields.io/badge/Chat-Slack-ff69b4.svg "Junte-se a nós. Todos são bem-vindos!")](http://slackhatesthe.cloud) ⇦ Junte-se a nós!

[Créditos](AUTHORS.md) ∙ [Diretrizes de Contribuição](CONTRIBUTING.md)
...
-	[Por que um Guia Aberto?](#why-an-open-guide)
-	[Escopo](#scope)
-	[Legenda](#legend)
...
-	[Aprendizado e Desenvolvimento de Carreira](#learning-and-career-development)
-	[Gerenciando AWS](#managing-aws)
-	[Gerenciando Servidores e Aplicações](#managing-servers-and-applications)
...
-	[Faturamento e Gerenciamento de Custos](#billing-and-cost-management)
...
[![Panorama do Mercado de Ferramentas e Serviços](figures/aws-market-landscape-320px.png)](#tools-and-services-market-landscape) [![Custos de Transferência de Dados da AWS](figures/aws-data-transfer-costs-320px.png)](#aws-data-transfer-costs)

-	[Figura: Panorama do Mercado de Ferramentas e Serviços](#tools-and-services-market-landscape): Uma seleção de empresas/produtos de terceiros
-	[Figura: Custos de Transferência de Dados da AWS](#aws-data-transfer-costs): Visão geral dos custos de transferência de dados
-	[Tabela: Matriz de Serviços](#service-matrix): Como os serviços da AWS se comparam com alternativas
-	[Tabela: Maturidade e Lançamentos de Produtos AWS](#aws-product-maturity-and-releases): Lançamentos de produtos AWS
-	[Tabela: Durabilidade, Disponibilidade e Preço do Armazenamento](#storage-durability-availability-and-price): Uma comparação quantitativa

Por que um Guia Aberto?
-----------------------

Muitas informações sobre a AWS já foram escritas. A maioria das pessoas aprende AWS lendo um blog ou um “[guia de início](http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/EC2_GetStarted.html)” e consultando as [referências padrão da AWS](https://aws.amazon.com/documentation/). No entanto, informações e recomendações confiáveis e práticas não são fáceis de encontrar. A documentação da AWS é um ótimo recurso, mas é extensa e poucos têm tempo para ler tudo, e não inclui nada além de fatos oficiais, omitindo experiências de engenheiros. As informações em blogs ou no [Stack Overflow](http://stackoverflow.com/questions/tagged/amazon-web-services) também não estão consistentemente atualizadas.

Este guia é feito por e para engenheiros que usam AWS. O objetivo é ser uma referência útil e dinâmica que consolida links, dicas, pegadinhas e melhores práticas. Ele surgiu de discussões e edições entre [vários engenheiros](AUTHORS.md) que usam AWS extensivamente.

Antes de usar o guia, leia por favor a [**licença**](#license) e o [**disclaimer**](#disclaimer).

[Voltar ao topo :arrow_up:](#table-of-contents)

### Por favor ajude!

**Este é um rascunho inicial em progresso!** É nossa primeira tentativa de compilar essas informações, portanto ainda está longe de ser abrangente e provavelmente tem omissões ou erros.

[![Slack Chat](https://img.shields.io/badge/Chat-Slack-ff69b4.svg "Junte-se a nós. Todos são bem-vindos!")](http://slackhatesthe.cloud)

Por favor, ajude [**entrando no canal Slack**](http://slackhatesthe.cloud) (gostamos de falar sobre AWS em geral, mesmo se você tiver apenas perguntas — a discussão ajuda a comunidade e guia melhorias) e [**contribuindo para o guia**](CONTRIBUTING.md). Este guia está *aberto a contribuições*, então, ao contrário de um blog, ele pode continuar melhorando. Como qualquer esforço de código aberto, combinamos esforços, mas também revisamos para garantir alta qualidade.

Escopo
------

-	Atualmente, este guia cobre serviços “centrais” selecionados, como EC2, S3, Load Balancers, EBS e IAM, e detalhes e dicas parciais sobre outros serviços. Esperamos que ele se expanda.
-	Não é um tutorial, mas sim uma coleção de informações que você pode ler e consultar novamente. É para iniciantes e experientes.
-	O objetivo deste guia é ser:
	-	**Breve:** Mantenha-o denso e use links
	-	**Prático:** Fatos básicos, detalhes concretos, conselhos, pegadinhas e outros “conhecimentos populares”
	-	**Atual:** Podemos mantê-lo atualizado, e qualquer pessoa pode contribuir com melhorias
	-	**Reflexivo:** O objetivo é ser útil em vez de apresentar fatos secos. Opiniões ponderadas com justificativa são bem-vindas. Sugestões, notas e opiniões baseadas em experiências reais podem ser extremamente valiosas. (Acreditamos que isso é possível com um guia nesse formato, ao contrário de alguns [outros locais](http://meta.stackexchange.com/questions/201994/is-there-a-place-to-ask-opinion-based-questions).)
-	Este guia não é patrocinado pela AWS ou por fornecedores afiliados à AWS. É escrito por e para engenheiros que usam AWS.

Legenda
------

-	📒 Marca páginas e documentos padrão/oficiais da AWS
-	🔹 Dica importante ou frequentemente esquecida
-	❗ “Pegadinha” séria (usado onde os riscos ou custos de tempo ou recursos são significativos: riscos críticos de segurança, erros com custo financeiro significativo ou escolhas arquitetônicas difíceis de corrigir)
-	🔸 “Pegadinha” regular, limitação ou peculiaridade (usado onde as consequências são coisas que não funcionam, quebram ou não escalam graciosamente)
-	📜 Recurso não documentado (folclore)
-	🐥 Serviços ou recursos relativamente novos (e talvez imaturos)
-	⏱ Discussões de desempenho
-	⛓ Lock-in: Produtos ou decisões que provavelmente irão amarrá-lo à AWS de uma maneira nova ou significativa — ou seja, mover-se posteriormente para uma alternativa não AWS seria custoso em termos de esforço de engenharia
-	🚪 Opções alternativas não AWS
-	💸 Questões de custo, discussões e pegadinhas
-	🕍 Um aviso leve ligado a “soluções completas” ou frameworks opinativos que podem levar um tempo significativo para entender e/ou podem não atender exatamente às suas necessidades; o oposto de uma solução pontual (a catedral é uma referência à [metáfora de Raymond](https://en.wikipedia.org/wiki/The_Cathedral_and_the_Bazaar)\)
-	📗📘📙 As cores indicam fundamentos, dicas e pegadinhas, respectivamente.
-	🚧 Áreas onde correção ou melhoria são necessárias (possivelmente com link para um problema — ajude se puder!)
...
-	[AWS](https://en.wikipedia.org/wiki/Amazon_Web_Services) é o provedor dominante de computação em nuvem pública.
	-	Em geral, “[computação em nuvem](https://en.wikipedia.org/wiki/Cloud_computing)” pode se referir a um dos três tipos de nuvem: “pública”, “privada” e “híbrida”. A AWS é um provedor de nuvem pública, já que qualquer pessoa pode usá-la. Nuvens privadas são dentro de uma única (geralmente grande) organização. Muitas empresas usam uma mistura de nuvens privadas e públicas.
	-	Os recursos principais da AWS são [infraestrutura como serviço](https://en.wikipedia.org/wiki/Cloud_computing#Infrastructure_as_a_service_.28IaaS.29) (IaaS) — ou seja, máquinas virtuais e infraestrutura de suporte. Outros modelos de serviço em nuvem incluem [plataforma como serviço](https://en.wikipedia.org/wiki/Cloud_computing#Platform_as_a_service_.28PaaS.29) (PaaS), que normalmente são serviços mais totalmente gerenciados que implantam as aplicações dos clientes, ou [software como serviço](https://en.wikipedia.org/wiki/Cloud_computing#Software_as_a_service_.28SaaS.29) (SaaS), que são aplicações baseadas em nuvem. A AWS também oferece alguns produtos que se encaixam nesses outros modelos.
	-	Em termos de negócios, com infraestrutura como serviço você tem um modelo de custo variável — é [OpEx, não CapEx](http://www.investopedia.com/ask/answers/020915/what-difference-between-capex-and-opex.asp) (embora alguns [contratos pré-adquiridos](https://aws.amazon.com/ec2/purchasing-options/reserved-instances/) ainda sejam CapEx).
	- A receita TTM da AWS foi de [**$37.549 bilhões**](https://ir.aboutamazon.com/news-release/news-release-details/2020/Amazoncom-Announces-First-Quarter/default.aspx) no primeiro trimestre de 2020, de acordo com seus resultados financeiros (slide 14 no deck vinculado), ou aproximadamente **14%** da receita total da Amazon.com (slide 11 no mesmo deck) para o mesmo período TTM.
-	**Principais razões para usar AWS:**
	- Se sua empresa está construindo sistemas ou produtos que podem precisar escalar
	- e você tem conhecimento técnico
	- e você quer as ferramentas mais flexíveis
	- e você não está significativamente amarrado a uma infraestrutura diferente
	- e você não tem razões internas, regulatórias ou de conformidade que impeçam o uso de uma solução baseada em nuvem pública
	- e você não está em um stack de tecnologia Microsoft-primeiro
	- e você não tem um motivo específico para usar o Google Cloud
	- e você pode pagar, gerenciar ou negociar seus custos um pouco mais altos
	- ... então a AWS provavelmente é uma boa opção para sua empresa.
-	Cada uma dessas razões acima pode apontar para situações onde outros serviços são preferíveis. Na prática, muitas, se não a maioria, das startups de tecnologia, bem como várias grandes empresas modernas, podem ou já se beneficiam do uso da AWS. Muitas grandes empresas estão migrando parte da infraestrutura interna para Azure, Google Cloud e AWS.
-	**Custos:** Faturamento e gerenciamento de custos são tópicos tão grandes que temos [uma seção inteira sobre isso](#billing-and-cost-management).
-	🔹**EC2 vs. outros serviços:** A maioria dos usuários da AWS está mais familiarizada com o [EC2](#ec2), o principal produto de servidor virtual da AWS, e possivelmente alguns outros como S3 e CLBs. Mas os produtos da AWS agora se estendem muito além do IaaS básico, e muitas vezes as empresas não entendem ou apreciam adequadamente todos os muitos serviços da AWS e como eles podem ser aplicados, devido ao [crescimento acentuado](#which-services-to-use) do número de serviços, sua novidade e complexidade, confusão de branding e medo de ⛓lock-in para tecnologia proprietária da AWS. Embora um pouco assustador, é importante para os tomadores de decisão técnica nas empresas entender a amplitude dos serviços da AWS e tomar decisões informadas. (Esperamos que este guia ajude.)
-	🚪**AWS vs. outros provedores de nuvem:** Embora a AWS seja o provedor dominante de IaaS (31% de participação de mercado nesta [estimativa de 2016](https://www.srgresearch.com/articles/aws-remains-dominant-despite-microsoft-and-google-growth-surges)), há uma concorrência significativa e alternativas que são mais adequadas para algumas empresas. [Este relatório da Gartner](https://www.gartner.com/doc/reprints?id=1-2G2O5FC&ct=150519&st=sb) tem uma boa visão geral dos principais players de nuvem:
	-	[**Google Cloud Platform**](https://cloud.google.com/). O GCP chegou ao mercado mais tarde que a AWS, mas tem vastos recursos e agora é amplamente utilizado por muitas empresas, incluindo algumas grandes. Está ganhando participação de mercado. Nem todos os serviços da AWS têm serviços semelhantes ou análogos no GCP. E vice-versa: em particular, o GCP oferece alguns serviços baseados em machine learning mais avançados, como as APIs [Vision](https://cloud.google.com/vision/), [Speech](https://cloud.google.com/speech/) e [Natural Language](https://cloud.google.com/natural-language/). Não é comum mudar uma vez que você está funcionando, mas isso acontece: [Spotify migrou](http://www.wsj.com/articles/google-cloud-lures-amazon-web-services-customer-spotify-1456270951) da AWS para o Google Cloud. Há mais discussões [no Quora](https://www.quora.com/What-are-the-reasons-to-choose-AWS-over-Google-Cloud-or-vice-versa-for-a-high-traffic-web-application) sobre benefícios relativos. De particular interesse é que os VPCs no GCP são [globais por padrão](https://cloud.google.com/vpc/) com sub-redes por região, enquanto os VPCs da AWS devem viver dentro de uma região específica. Isso dá ao GCP uma vantagem se você estiver projetando aplicações com geo-replicação desde o início. Também é possível [compartilhar um VPC do GCP](https://cloud.google.com/compute/docs/shared-vpc/) entre vários projetos (mais ou menos análogo a contas da AWS), enquanto na AWS você teria que pareá-los. Também é possível [parear VPCs do GCP](https://cloud.google.com/compute/docs/vpc/vpc-peering) de maneira semelhante à como é feito na AWS.
	-	[**Microsoft Azure**](https://azure.microsoft.com/en) é a escolha padrão para empresas e equipes focadas em um stack Microsoft, e agora colocou ênfase significativa no Linux também.
	- Na **China**, a presença da AWS é relativamente pequena. O mercado é dominado pela [Alibaba Cloud](https://www.alibabacloud.com/) da Alibaba, anteriormente chamada [Aliyun](https://intl.aliyun.com/).
	- Empresas em grande escala podem querer reduzir custos gerenciando sua própria infraestrutura. Por exemplo, [Dropbox migrou](https://news.ycombinator.com/item?id=11282948) para sua própria infraestrutura.
	- Outros provedores de nuvem, como [Digital Ocean](https://www.digitalocean.com/), oferecem serviços semelhantes, às vezes com maior facilidade de uso, suporte mais personalizado ou menor custo. No entanto, nenhum desses iguala a amplitude de produtos, reconhecimento de marca e domínio de mercado que a AWS desfruta agora.
	- Provedores de hospedagem gerenciada tradicionais, como [Rackspace](https://www.rackspace.com/), também oferecem soluções em nuvem.
-	🚪**AWS vs. PaaS:** Se o seu objetivo é apenas colocar um único serviço que faça algo relativamente simples e você está tentando minimizar o tempo gerenciando a engenharia de operações, considere uma [plataforma como serviço](https://en.wikipedia.org/wiki/Platform_as_a_service) como [Heroku](https://www.heroku.com/). A abordagem da AWS para PaaS, Elastic Beanstalk, é indiscutivelmente mais complexa, especialmente para casos de uso simples.
-	🚪**AWS vs. hospedagem web:** Se seu objetivo principal é hospedar um site ou blog e você não espera construir um aplicativo ou serviço mais complexo, talvez queira considerar um dos inúmeros [serviços de hospedagem web](https://www.google.com/search?q=web+hosting).
-	🚪**AWS vs. hospedagem gerenciada:** Tradicionalmente, muitas empresas pagam [provedores de hospedagem gerenciada](https://en.wikipedia.org/wiki/Dedicated_hosting_service) para manter servidores físicos para elas, então constroem e implantam seu software em cima do hardware alugado. Isso faz sentido para empresas que querem controle direto sobre o hardware, devido a legados, desempenho ou restrições especiais de conformidade, mas geralmente é considerado antiquado ou desnecessário por muitas startups centradas em desenvolvedores e empresas de tecnologia mais jovens.
-	**Complexidade:** A AWS permitirá que você construa e escale sistemas para o tamanho das maiores empresas, mas a complexidade dos serviços quando usados em escala requer conhecimento e experiência significativos. Mesmo casos de uso muito simples muitas vezes requerem mais conhecimento para fazer “certo” na AWS do que em um ambiente mais simples como Heroku ou Digital Ocean. (Este guia pode ajudar!)
-	**Localizações geográficas:** A AWS tem data centers em [mais de uma dúzia de localizações geográficas](http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-regions-availability-zones.html#concepts-available-regions), conhecidas como **regiões**, na Europa, Leste Asiático, América do Norte e do Sul e agora Austrália e Índia. Também possui muitas outras **localizações de borda** globalmente para reduzir a latência de serviços como CloudFront.
	-	Veja a [lista atual](https://aws.amazon.com/about-aws/global-infrastructure/) de regiões e localizações de borda, incluindo as próximas.
	-	Se sua infraestrutura precisar estar em proximidade física com outro serviço por razões de latência ou throughput (por exemplo, latência para uma bolsa de anúncios), a viabilidade da AWS pode depender da localização.
-	⛓**Lock-in:** À medida que você usa a AWS, é importante estar ciente de quando você está dependendo de serviços da AWS que não têm equivalentes em outros lugares.
	-	O lock-in pode ser completamente aceitável para sua empresa ou um risco significativo. É importante, do ponto de vista empresarial, fazer essa escolha explicitamente e considerar os custos, operacionais, de continuidade dos negócios e de riscos competitivos de estar amarrado à AWS. A AWS é um fornecedor tão dominante e confiável que muitas empresas se sentem confortáveis em usar a AWS em toda sua extensão. Outras podem contar histórias sobre os [perigos de “prisão na nuvem” quando os custos disparam](http://firstround.com/review/the-three-infrastructure-mistakes-your-company-must-not-make/).
	-	Geralmente, quanto mais serviços AWS você usa, mais lock-in você tem com a AWS — ou seja, mais recursos de engenharia (tempo e dinheiro) serão necessários para mudar para outros provedores no futuro.
	- Serviços básicos como servidores virtuais e bancos de dados padrão são geralmente fáceis de migrar para outros provedores ou para ambientes locais. Outros como balanceadores de carga e IAM são específicos da AWS, mas têm equivalentes próximos de outros provedores. A questão chave a considerar é se os engenheiros estão arquitetando sistemas em torno de serviços específicos da AWS que não são open source ou relativamente intercambiáveis. Por exemplo, Lambda, API Gateway, Kinesis, Redshift e DynamoDB não têm equivalentes open source ou de serviços comerciais substancialmente equivalentes, enquanto EC2, RDS (MySQL ou Postgres), EMR e ElastiCache mais ou menos têm. (Veja mais [abaixo](#which-services-to-use), onde estes são anotados com ⛓.)
-	**Combinando AWS e outros provedores de nuvem:** Muitos clientes combinam AWS com outros serviços não AWS. Por exemplo, sistemas legados ou dados seguros podem estar em um provedor de hospedagem gerenciada, enquanto outros sistemas estão na AWS. Ou uma empresa pode usar apenas o S3 com outro provedor fazendo todo o resto. No entanto, pequenas startups ou projetos iniciantes normalmente ficarão apenas com AWS ou Google Cloud.
-	**Nuvem híbrida:** Em grandes empresas, é comum ter [implantações híbridas](https://aws.amazon.com/enterprise/hybrid/) abrangendo nuvens privadas ou servidores on-premises e AWS — ou outros provedores de nuvem empresarial como [IBM](https://www.ibm.com/it-infrastructure/solutions/hybrid-cloud)/[Bluemix](http://www.ibm.com/cloud-computing/bluemix/hybrid/), [Microsoft](https://www.microsoft.com/en-us/cloud-platform/hybrid-cloud)/[Azure](https://azure.microsoft.com/en-us/overview/azure-stack/), [NetApp](http://www.netapp.com/us/solutions/cloud/hybrid-cloud/), ou [EMC](http://www.emc.com/en-us/cloud/hybrid-cloud-computing/index.htm).
-	**Principais clientes:** Quem usa AWS e Google Cloud?
	-	A [lista de clientes](https://aws.amazon.com/solutions/case-studies/) da AWS inclui um grande número de propriedades online e grandes marcas, como Netflix, Pinterest, Spotify (movendo-se para Google Cloud), Airbnb, Expedia, Yelp, Zynga, Comcast, Nokia e Bristol-Myers Squibb.
	-	A [lista de clientes](https://azure.microsoft.com/en-us/case-studies/) da Azure inclui empresas como NBC Universal, 3M e Honeywell Inc.
	-	A [lista de clientes](https://cloud.google.com/customers/) do Google Cloud também é grande, e inclui alguns sites conhecidos, como [Snapchat](http://www.businessinsider.com/snapchat-is-built-on-googles-cloud-2014-1), Best Buy, Domino’s e Sony Music.

[Voltar ao topo :arrow_up:](#table-of-contents)

### Quais Serviços Usar

-	A AWS oferece *muitos* serviços diferentes — [cerca de cem](https://aws.amazon.com/products/) na última contagem.
-	A maioria dos clientes usa alguns serviços intensivamente, alguns serviços levemente e o restante não usa. Os serviços que você utilizará dependem de seus casos de uso. As escolhas variam substancialmente de empresa para empresa.
-	**Serviços imaturos e impopulares:** Só porque a AWS tem um serviço que parece promissor, não significa que você deve usá-lo. Alguns serviços têm casos de uso muito específicos, não são maduros, são excessivamente opinativos ou têm limitações, então construir sua própria solução pode ser melhor. Tentamos dar uma noção disso dividindo os produtos em categorias.
-	**Infraestrutura essencial:** A maioria dos usuários típicos de pequeno a médio porte se concentrará nos seguintes serviços primeiro. Se você gerencia o uso de sistemas AWS, provavelmente precisará saber pelo menos um pouco sobre todos eles. (Mesmo que você não os use, deve aprender o suficiente para fazer essa escolha de maneira inteligente.)
	-	[IAM](#security-and-iam): Contas de usuário e identidades (você precisa pensar nas contas desde o início!)
	-	[EC2](#ec2): Servidores virtuais e componentes associados, incluindo:
		-	[AMIs](#amis): Imagens de Máquina
		-	[Load Balancers](#load-balancers): CLBs e ALBs
		-	[Autoscaling](#auto-scaling): Escalabilidade de capacidade (adicionar e remover servidores com base na carga)
		-	[EBS](#ebs): Discos conectados à rede
		-	[Elastic IPs](#elastic-ips): Endereços IP atribuídos
	-	[S3](#s3): Armazenamento de arquivos
	-	[Route 53](#route-53): DNS e registro de domínio
	-	[VPC](#vpcs-network-security-and-security-groups): Rede virtual, segurança de rede e co-localização; você usa automaticamente
	-	[CloudFront](#cloudfront): CDN para hospedagem de conteúdo
	-	[CloudWatch](#cloudwatch): Alertas, paginação, monitoramento
-	**Serviços gerenciados:** Soluções de software existentes que você poderia executar por conta própria, mas com implantação gerenciada:
	-	[RDS](#rds): Bancos de dados relacionais gerenciados (MySQL gerenciado, Postgres e o próprio banco de dados Aurora da Amazon)
	-	[EMR](#emr): Hadoop gerenciado
	-	[Elasticsearch](https://aws.amazon.com/elasticsearch-service/): Elasticsearch gerenciado
	-	[ElastiCache](https://aws.amazon.com/elasticache/): Redis e Memcached gerenciados
-	**Infraestrutura opcional mas importante:** Estes são componentes de infraestrutura essenciais e úteis que são menos amplamente conhecidos e utilizados. Você pode ter motivos legítimos para preferir alternativas, então avalie com cuidado para ter certeza de que atendem às suas necessidades:
	-	⛓[Lambda](#lambda): Execução de pequenas tarefas totalmente gerenciadas "serverless"
	-	[CloudTrail](https://aws.amazon.com/cloudtrail/): Registro e auditoria de API AWS (frequentemente negligenciado, mas importante)
	-	⛓🕍[CloudFormation](#cloudformation): Configuração template de coleções de recursos AWS
	-	🕍[Elastic Beanstalk](https://aws.amazon.com/elasticbeanstalk/): Implantação totalmente gerenciada (PaaS) de aplicações Java, .NET, PHP, Node.js, Python, Ruby, Go e Docker
	-	🐥[EFS](#efs): Sistema de arquivos de rede compatível com NFSv4.1
	-	⛓🕍[ECS](#ecs): Gerenciamento de contêineres/cluster Docker (observe que o Docker também pode ser usado diretamente, sem o ECS)
	-   🕍[EKS](#eks): Gerenciamento de Contêiner/Cluster Docker Kubernetes (K8)
	-	⛓[ECR](https://aws.amazon.com/ecr/): Registro Docker privado hospedado
	-	🐥[Config](https://aws.amazon.com/config/): Inventário de configuração da AWS, histórico, notificações de mudanças
	-	🐥[X-Ray](https://aws.amazon.com/xray/): Análise de rastreamento e depuração para aplicações distribuídas como microserviços.
-	**Infraestrutura de propósito específico:** Esses serviços são focados em casos de uso específicos e devem ser avaliados se aplicáveis à sua situação. Muitos também são arquiteturas proprietárias, então tendem a amarrá-lo à AWS.
	-	⛓[DynamoDB](#dynamodb): Armazenamento NoSQL de baixa latência
	-	⛓[Glacier](#glacier): Alternativa lenta e barata ao S3
	-	⛓[Kinesis](https://aws.amazon.com/kinesis/): Serviço de streaming (log distribuído)
	-	⛓[SQS](https://aws.amazon.com/sqs/): Serviço de filas de mensagens
	-	⛓[Redshift](#redshift): Armazém de dados
	-	🐥[QuickSight](https://aws.amazon.com/quicksight/): Serviço de inteligência de negócios
	-	[SES](https://aws.amazon.com/ses/): Envio e recebimento de e-mails para marketing ou transações
	-	⛓[API Gateway](https://aws.amazon.com/api-gateway/): Proxy, gerencia e segura chamadas de API
	-	⛓[IoT](#iot): Gerenciamento de comunicação bidirecional sobre HTTP, WebSockets e MQTT entre AWS e clientes (frequentemente, mas não necessariamente “coisas” como eletrodomésticos ou sensores)
	-	⛓[WAF](https://aws.amazon.com/waf/): Firewall web para CloudFront para desviar ataques
	-	⛓[KMS](#kms): Armazenar e gerenciar chaves de criptografia com segurança
	-	[Inspector](https://aws.amazon.com/inspector/): Auditoria de segurança
	-	[Trusted Advisor](https://aws.amazon.com/premiumsupport/trustedadvisor/): Dicas automatizadas sobre redução de custos ou melhorias
	-	🐥[Certificate Manager](https://aws.amazon.com/certificate-manager/): Gerenciar certificados SSL/TLS para serviços AWS
	-	🐥⛓[Fargate](https://aws.amazon.com/fargate/): Gerenciamento de contêineres Docker, backend para ECS e EKS
-	**Serviços compostos:** Estes são igualmente específicos, mas são serviços completos que abordam problemas complexos e podem amarrá-lo. A utilidade depende de seus requisitos. Se você tem uma necessidade grande ou significativa, pode já ter esses serviços gerenciados por sistemas e equipes de engenharia internos.
	-	[Machine Learning](https://aws.amazon.com/machine-learning/): Treinamento de modelos de machine learning e classificação
	-	[Lex](https://aws.amazon.com/lex/): Reconhecimento automático de fala (ASR) e entendimento de linguagem natural (NLU)
	-	[Polly](https://aws.amazon.com/polly/): Motor de conversão de texto em fala na nuvem
	-	[Rekognition](https://aws.amazon.com/rekognition/): Serviço de reconhecimento de imagem
	-	⛓🕍[Data Pipeline](https://aws.amazon.com/datapipeline/): Serviço gerenciado de ETL
	-	⛓🕍[SWF](https://aws.amazon.com/swf/): Rastreador de estado gerenciado para fluxo de trabalho de tarefas distribuídas poliglotas
	-	⛓🕍[Lumberyard](https://aws.amazon.com/lumberyard/): Motor de jogo 3D
-	**Desenvolvimento móvel/aplicativo:**
	-	[SNS](https://aws.amazon.com/sns/): Gerenciar notificações push de aplicativos e outras notificações para usuários finais
	-	⛓🕍[Cognito](https://aws.amazon.com/cognito/): Autenticação de usuário via Facebook, Twitter, etc.
	-	[Device Farm](https://aws.amazon.com/device-farm/): Teste de dispositivos baseado na nuvem
	-	[Mobile Analytics](https://aws.amazon.com/mobileanalytics/): Solução de análise para uso de aplicativos
	-	🕍[Mobile Hub](https://aws.amazon.com/mobile/): Framework abrangente e gerenciado para aplicativos móveis
-	**Serviços empresariais:** Estes são relevantes se você tiver necessidades corporativas significativas baseadas na nuvem ou híbridas. Muitas empresas menores e startups usam outras soluções, como Google Apps ou Box. Empresas maiores também podem ter suas próprias soluções de TI não AWS.
	-	[AppStream](https://aws.amazon.com/appstream/): Aplicativos Windows na nuvem, com acesso de vários dispositivos
	-	[Workspaces](https://aws.amazon.com/workspaces/): Desktop Windows na nuvem, com acesso de vários dispositivos
	-	[WorkDocs](https://aws.amazon.com/workdocs/) (anteriormente Zocalo): Compartilhamento de documentos empresariais
	-	[WorkMail](https://aws.amazon.com/workmail/): Serviço de e-mail e calendário gerenciado para empresas
	-	[Directory Service](https://aws.amazon.com/directoryservice/): Microsoft Active Directory na nuvem
	-	[Direct Connect](https://aws.amazon.com/directconnect/): Conexão de rede dedicada entre escritório ou data center e AWS
	-	[Storage Gateway](https://aws.amazon.com/storagegateway/): Ponte entre TI local e armazenamento na nuvem
	-	[Service Catalog](https://aws.amazon.com/servicecatalog/): Aprovação e conformidade de serviços de TI
-	**Serviços que provavelmente você não precisa conhecer:** Em resumo, nossa pesquisa informal indica que esses serviços simplesmente não são amplamente utilizados — e muitas vezes por bons motivos:
	-	[Snowball](https://aws.amazon.com/importexport/): Se você quiser enviar petabytes de dados para dentro ou fora da Amazon usando um appliance físico, continue lendo.
	-	[Snowmobile](https://aws.amazon.com/snowmobile/): Appliances são ótimos, mas se você tiver dados em escala de exabyte para colocar na Amazon, nada supera um caminhão cheio de drives.
	-	[CodeCommit](https://aws.amazon.com/codecommit/): Serviço Git. Você provavelmente já está usando GitHub ou sua própria solução ([Stackshare](http://stackshare.io/stackups/github-vs-bitbucket-vs-aws-codecommit) tem estatísticas informais).
	-	🕍[CodePipeline](https://aws.amazon.com/codepipeline/): Integração contínua. Você provavelmente já tem outra solução.
	-	🕍[CodeDeploy](https://aws.amazon.com/codedeploy/): Implantação de código em servidores EC2. Novamente, você provavelmente já tem outra solução.
	-	🕍[OpsWorks](https://aws.amazon.com/opsworks/): Gerenciamento de suas implantações usando Chef ou (a partir de novembro de 2017) Puppet Enterprise.
-	[AWS em Inglês Simples](https://www.expeditedssl.com/aws-in-plain-english) oferece uma explicação mais amigável do que são todos os diferentes serviços.

[Voltar ao topo :arrow_up:](#table-of-contents)

### Panorama do Mercado de Ferramentas e Serviços

Agora existem empresas e produtos suficientes de nuvem e “big data” para que poucos possam acompanhar o panorama do mercado.

Montamos um panorama de alguns dos serviços. Está longe de ser completo, mas tenta enfatizar serviços que são populares entre os praticantes de AWS — serviços que ajudam especificamente com a AWS, são complementares, ou ferramentas que quase todos que usam a AWS devem aprender.

![Ferramentas e Serviços Populares para Praticantes de AWS](figures/aws-market-landscape.png)

🚧 *Sugestões para melhorar esta figura? Por favor, [registre um problema](CONTRIBUTING.md).*

[Voltar ao topo :arrow_up:](#table-of-contents)

### Conceitos Comuns

-	📒 A [**Referência Geral**](https://docs.aws.amazon.com/general/latest/gr/Welcome.html) da AWS cobre vários conceitos comuns que são relevantes para múltiplos serviços.
-	A AWS permite implantações em [**regiões**](https://docs.aws.amazon.com/general/latest/gr/rande.html), que são localizações geográficas isoladas que ajudam a reduzir a latência ou oferecem redundância adicional. As regiões contêm zonas de disponibilidade (AZs), que normalmente são a primeira escolha para [alta disponibilidade](#high-availability). As AZs são [fisicamente separadas umas das outras](https://www.youtube.com/watch?v=JIQETrFC_SQ&feature=youtu.be&t=1428) mesmo dentro da mesma região, e [podem abranger vários data centers físicos](https://blog.rackspace.com/aws-101-regions-availability-zones). Embora sejam conectadas por links de baixa latência, desastres naturais que afetam uma não devem afetar outras.
-	Cada serviço tem **endpoints** de API para cada região. Os endpoints diferem de serviço para serviço e nem todos os serviços estão disponíveis em cada região, conforme listado nestas [tabelas](https://docs.aws.amazon.com/general/latest/gr/rande.html).
-	[**Amazon Resource Names (ARNs)**](https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) são identificadores especialmente formatados para identificar recursos. Eles começam com 'arn:' e são usados em muitos serviços, em particular para políticas IAM.

[Voltar ao topo :arrow_up:](#table-of-contents)

### Matriz de Serviços

Muitos serviços dentro da AWS podem, pelo menos, ser comparados com ofertas do Google Cloud ou com serviços internos do Google. E muitas vezes você pode montar a mesma coisa com software de código aberto. Esta tabela é um esforço para listar essas correspondências aproximadas. (Lembre-se de que esta tabela é imperfeita, pois em quase todos os casos existem diferenças sutis de recursos!)
...
🚧 [*Por favor, ajude a preencher esta tabela.*](CONTRIBUTING.md)

Recursos selecionados com mais detalhes sobre este gráfico:

-	Google interno: [MapReduce](http://research.google.com/archive/mapreduce.html), [Bigtable](http://research.google.com/archive/bigtable.html), [Spanner](http://research.google.com/archive/spanner.html), [F1 vs Spanner](http://highscalability.com/blog/2013/10/8/f1-and-spanner-holistically-compared.html), [Bigtable vs Megastore](http://perspectives.mvdirona.com/2008/07/google-megastore/)

[Voltar ao topo :arrow_up:](#table-of-contents)

### Maturidade e Lançamentos de Produtos AWS

É importante conhecer a maturidade de cada produto AWS. Aqui está uma lista quase completa da data de lançamento inicial, com links para as [notas de lançamento](https://aws.amazon.com/releasenotes/). Os serviços lançados mais recentemente estão primeiro. Nem todos os serviços estão disponíveis em todas as regiões; veja [esta tabela](https://aws.amazon.com/about-aws/global-infrastructure/regional-product-services/).
...
##### Notas de Rodapé
<a name="user-content-hipaa-apigateway">**1**</a>: Exclui o uso de cache do Amazon API Gateway<br />
<a name="user-content-hipaa-rds">**2**</a>: Somente os motores RDS MySQL, Oracle e PostgreSQL<br />
<a name="user-content-hipaa-aurora">**3**</a>: Somente a edição Aurora compatível com MySQL<br />
<a name="user-content-hipaa-cloudfront">**4**</a>: Exclui Lambda@Edge<br />
<a name="user-content-hipaa-ec2sysmgr">**5**</a>: Inclui o EC2 Systems Manager<br />
<a name="user-content-hipaa-ec2ebs">**6**</a>: Inclui o Elastic Block Storage (EBS)<br />
<a name="user-content-hipaa-ec2elb">**7**</a>: Inclui o Elastic Load Balancing<br />
<a name="user-content-hipaa-s3">**8**</a>: Inclui a Aceleração de Transferência S3<br />
<a name="user-content-pci-rds">**9**</a>: Inclui RDS MySQL, Oracle, PostgreSQL, SQL Server e MariaDB</br>
<a name="user-content-pci-asg">**10**</a>: Inclui Auto-Scaling</br>
<a name="user-content-pci-kinesis">**11**</a>: Data Analytics, Streams, Video Streams e Firehose</br>
<a name="user-content-eks-cli">**12**</a>: O Kubernetes usa uma CLI personalizada para gerenciamento de Pods/Serviços chamada kubectl. A AWS CLI lida apenas com preocupações do Kubernetes Master</br>
<a name="user-content-pci-iot">**13**</a>: IoT Core (inclui Gerenciamento de Dispositivos) e Greengrass</br>
<a name="user-content-pci-elasticache">**14**</a>: Somente ElastiCache para Redis</br>
<a name="user-content-pci-snowball">**15**</a>: Snowball e Snowball Edge</br>

### Conformidade

-	Muitas aplicações têm requisitos rigorosos em relação à confiabilidade, segurança ou privacidade de dados. A página de [Conformidade da AWS](https://aws.amazon.com/compliance/) tem detalhes sobre as certificações da AWS, que incluem **PCI DSS Nível 1**, **SOC 1, 2 e 3**, **HIPAA** e **ISO 9001**.
-	A segurança na nuvem é um tópico complexo, baseado em um [modelo de responsabilidade compartilhada](https://aws.amazon.com/compliance/shared-responsibility-model/), onde alguns elementos de conformidade são fornecidos pela AWS, e outros são fornecidos pela sua empresa.
-	Vários fornecedores terceirizados oferecem assistência com conformidade, segurança e auditoria na AWS. Se você tiver necessidades substanciais nessas áreas, buscar assistência é uma boa ideia.
-	De dentro da **China**, os serviços da AWS fora da China [são geralmente acessíveis](https://en.greatfire.org/aws.amazon.com), embora às vezes haja interrupções no serviço. Também existem serviços da AWS [dentro da China](https://www.amazonaws.cn/en/).

### Obtenção de Ajuda e Suporte

-	**Fóruns:** Para muitos problemas, vale a pena pesquisar ou pedir ajuda nos [fóruns de discussão](https://forums.aws.amazon.com/index.jspa) para ver se é um problema conhecido.
-	**Suporte premium:** A AWS oferece vários níveis de [suporte premium](https://aws.amazon.com/premiumsupport/).
	-	O primeiro nível, chamado "Suporte para Desenvolvedores", permite que você registre tickets de suporte com tempo de resposta de 12 a 24 horas, começa em $29, mas quando seu gasto mensal atinge cerca de $1000, muda para um encargo de 3% na sua conta.
	-	Os serviços de suporte de nível superior são bastante caros — e aumentam sua conta em até 10%. Muitas grandes empresas eficazes nunca pagam por este nível de suporte. Eles geralmente são mais úteis para empresas de médio ou grande porte que precisam de uma resposta rápida para problemas mais profundos ou mais complexos.
	-	Lembre-se, uma arquitetura flexível pode reduzir a necessidade de suporte. Você não deve depender da AWS para resolver seus problemas com frequência. Por exemplo, se você pode provisionar facilmente um novo servidor, pode não ser urgente resolver um problema raro de nível de kernel exclusivo de uma instância EC2. Se seus volumes EBS tiverem snapshots recentes, você poderá restaurar um volume antes que o suporte possa resolver o problema com o volume antigo. Se seus serviços tiverem um problema em uma zona de disponibilidade, você deve, em qualquer caso, ser capaz de contar com uma zona redundante ou migrar serviços para outra zona.
	-	Clientes maiores também têm acesso ao suporte Enterprise da AWS, com gerentes de contas técnicas dedicados (TAMs) e SLAs de tempo de resposta mais curtos.
	-	Há definitivamente alguma controvérsia sobre a utilidade do suporte pago. Os funcionários de suporte nem sempre parecem ter as informações e a autoridade para resolver os problemas que são trazidos à sua atenção. Muitas vezes, sua capacidade de resolver um problema pode depender do seu relacionamento com seu representante de conta.
-	**Gerente de conta:** Se você tiver níveis significativos de gastos (milhares de dólares americanos por mês), pode ser designado (ou pode desejar pedir) um gerente de conta dedicado.
	-	Esses são um grande recurso, mesmo se você não estiver pagando pelo suporte premium. Construa um bom relacionamento com eles e use-os para perguntas, problemas e orientações.
	-	Designe um ponto de contato único do lado da sua empresa, para evitar confundi-los ou sobrecarregá-los.
-	**Contato:** O principal ponto de contato na web para AWS está [aqui](https://aws.amazon.com/contact-us/). Muitas solicitações técnicas podem ser feitas por esses canais.
-	**Consultoria e serviços gerenciados:** Para assistência mais prática, a AWS tem relações estabelecidas com muitos [parceiros de consultoria](https://aws.amazon.com/partners/consulting/) e [parceiros de serviços gerenciados](https://aws.amazon.com/partners/msp/). Os grandes consultores não serão baratos, mas dependendo de suas necessidades, podem economizar custos a longo prazo, ajudando você a configurar sua arquitetura de maneira mais eficaz ou oferecendo expertise específica, por exemplo, segurança. Provedores de serviços gerenciados fornecem gerenciamento de recursos em nuvem a longo prazo e de serviço completo.
-	**AWS Professional Services:** A AWS fornece [serviços de consultoria](https://aws.amazon.com/professional-services/) sozinha ou em combinação com parceiros.

[Voltar ao topo :arrow_up:](#table-of-contents)

### Restrições e Outras Notas

-	🔸Muitos recursos da Amazon têm [**limites**](http://docs.aws.amazon.com/general/latest/gr/aws_service_limits.html). Isso é útil, pois evita que você incorra em grandes custos acidentalmente. Você deve solicitar o aumento das cotas abrindo tickets de suporte. Alguns limites são fáceis de aumentar, e outros não. (Alguns destes são mencionados nas seções abaixo.) Além disso, nem todos os limites de serviço são publicados.
	- **Obtendo Limites e Uso Atuais:** Informações sobre limites de um serviço podem estar disponíveis na API do serviço, no Trusted Advisor, em ambos ou em nenhum (neste caso, você precisará entrar em contato com o Suporte). [Esta página](http://awslimitchecker.readthedocs.io/en/latest/limits.html) da documentação da ferramenta awslimitchecker fornece um bom resumo das opções de recuperação disponíveis para cada limite. A própria [ferramenta](https://github.com/jantman/awslimitchecker) também é valiosa para automatizar verificações de limites.
-	🔸Os [**termos de serviço da AWS**](https://aws.amazon.com/service-terms/) são extensos. Muito é padrão esperado, mas contém notas importantes e restrições em cada serviço. Em particular, há restrições contra o uso de muitos serviços da AWS em **sistemas críticos de segurança**. (Aqueles que apreciam humor jurídico podem querer revisar a [cláusula 42.10](https://www.theguardian.com/technology/2016/feb/11/amazon-terms-of-service-zombie-apocalypse).)

### Tópicos Relacionados

-	[OpenStack](https://www.openstack.org/) é uma alternativa de nuvem privada à AWS usada por grandes empresas que desejam evitar ofertas de nuvem pública.
...
-	**Certificações:** A AWS oferece [**certificações**](https://aws.amazon.com/certification/) para profissionais de TI que desejam demonstrar seu conhecimento.
...
As certificações de nível Associate já foram exigidas como pré-requisitos para os exames de nível Professional - isso não é mais o caso.

-	**Obter a certificação:** Se você está interessado em estudar e obter certificações, [esta visão geral prática](https://gist.github.com/leonardofed/bbf6459ad154ad5215d354f3825435dc) diz muito do que você precisa saber. A página oficial está [aqui](https://aws.amazon.com/training/) e há uma [FAQ](https://aws.amazon.com/certification/faqs/).
-	**Treinamento para certificações:** O treinamento é oferecido pela própria AWS (principalmente presencial e liderado por instrutores) e por várias empresas terceirizadas (geralmente como treinamento em vídeo), como [A Cloud Guru](https://acloud.guru/aws-cloud-training), [CloudAcademy](https://cloudacademy.com/library/amazon-web-services/) e [Linux Academy](https://linuxacademy.com/library/topics/AWS/type/Course/).
-	**Você precisa de uma certificação?** Especialmente em empresas de consultoria ou ao trabalhar em funções técnicas-chave em grandes empresas não tecnológicas, as certificações são credenciais importantes. Em outras, incluindo muitas empresas de tecnologia e startups, certificações não são comuns ou consideradas necessárias. (De fato, justa ou injustamente, alguns gerentes de contratação e engenheiros do Vale do Silício as veem como um sinal “negativo” em um currículo.)

As certificações são necessárias para acessar os lounges de certificados em eventos oficiais da AWS, como [Summits](https://aws.amazon.com/events/summits/) e [re:Invent](https://reinvent.awsevents.com). Os lounges geralmente oferecem pontos de recarga de energia, assentos e café relativamente melhor.

Gerenciando AWS
---------------

### Gerenciando o Estado e as Mudanças na Infraestrutura

Um grande desafio ao usar a AWS para construir sistemas complexos (e com DevOps em geral) é gerenciar o estado da infraestrutura de maneira eficaz ao longo do tempo. Em geral, isso se resume a três grandes objetivos para o estado da sua infraestrutura:

-	**Visibilidade**: Você conhece o estado da sua infraestrutura (quais serviços você está usando e exatamente como)? Você também sabe quando você — e qualquer pessoa da sua equipe — faz mudanças? Você pode detectar configurações incorretas, problemas e incidentes com seu serviço?
-	**Automação**: Você pode reconfigurar sua infraestrutura para reproduzir configurações anteriores ou expandir as existentes sem muito trabalho manual extra ou exigindo conhecimento que está apenas na cabeça de alguém? Você pode responder a incidentes facilmente ou automaticamente?
-	**Flexibilidade**: Você pode melhorar suas configurações e expandir de novas maneiras sem esforço significativo? Você pode adicionar mais complexidade usando as mesmas ferramentas? Você compartilha, revisa e melhora suas configurações dentro da sua equipe?

Muito do que discutimos abaixo é realmente sobre como melhorar as respostas a essas perguntas.

Existem várias abordagens para implantar infraestrutura com a AWS, desde o console até ferramentas complexas de automação, passando por serviços de terceiros, todos os quais tentam ajudar a alcançar visibilidade, automação e flexibilidade.

### Gerenciamento de Configuração AWS

A primeira maneira que a maioria das pessoas experimenta a AWS é por meio de sua interface web, o Console da AWS. Mas usar o Console é um processo altamente manual e muitas vezes vai contra a automação ou flexibilidade.

Então, se você não vai gerenciar suas configurações da AWS manualmente, o que você deve fazer? Infelizmente, não há respostas simples e universais — cada abordagem tem prós e contras, e as abordagens adotadas por diferentes empresas variam amplamente, e incluem usar diretamente APIs (e construir ferramentas por conta própria), usar ferramentas de linha de comando e usar ferramentas e serviços de terceiros.

### Console AWS

-	O [Console AWS](https://aws.amazon.com/console/) permite controlar grande parte (mas não toda) a funcionalidade da AWS por meio de uma interface web.
-	Idealmente, você deve usar o Console AWS apenas em algumas situações específicas:
	-	É ótimo para uso apenas de leitura. Se você está tentando entender o estado do seu sistema, fazer login e navegar nele é muito útil.
	-	Também é razoavelmente viável para sistemas e equipes muito pequenos (por exemplo, um engenheiro configurando um servidor que não muda com frequência).
	-	Pode ser útil para operações que você vai fazer raramente, como menos de uma vez por mês (por exemplo, uma configuração única de VPC que você provavelmente não revisitará por um ano). Nesse caso, usar o console pode ser a abordagem mais simples.
-	❗**Pense antes de usar o console:** O Console da AWS é conveniente, mas também é o inimigo da automação, reprodutibilidade e comunicação da equipe. Se você provavelmente fará a mesma alteração várias vezes, evite o console. Prefira algum tipo de automação, ou pelo menos tenha um caminho para a automação, como discutido a seguir. Usar o console não só impede a automação, o que desperdiça tempo mais tarde, mas também impede a documentação, clareza e padronização em torno dos processos para você e sua equipe.

### Ferramentas de Linha de Comando

-	A [**interface de linha de comando da AWS**](https://aws.amazon.com/cli/) (CLI), usada através do comando **aws**, é a maneira mais básica de salvar e automatizar operações na AWS.
-	Não subestime seu poder. Também tem a vantagem de ser bem mantida — cobre uma grande parte de todos os serviços da AWS e está atualizada.
-	Em geral, sempre que possível, prefira a linha de comando ao Console da AWS para realizar operações.
-	🔹Mesmo na ausência de ferramentas mais sofisticadas, você pode **escrever scripts Bash simples** que invocam *aws* com argumentos específicos e registrar esses scripts no Git. Esta é uma maneira primitiva, mas eficaz, de documentar as operações que você realizou. Isso melhora a automação, permite revisão de código e compartilhamento em uma equipe e dá aos outros um ponto de partida para trabalhos futuros.
-	🔹Para uso que é principalmente interativo (não scriptado), considere usar a ferramenta [**aws-shell**](https://github.com/awslabs/aws-shell) da AWS. É mais fácil de usar, com autocompletar e uma interface colorida, mas ainda funciona na linha de comando. Se você está usando [SAWS](https://github.com/donnemartin/saws), uma versão anterior do programa, [você deve migrar para aws-shell](https://github.com/donnemartin/saws/issues/68#issuecomment-240067034).

### APIs e SDKs

-	**SDKs** para usar as APIs da AWS estão disponíveis na maioria das principais linguagens, sendo [Go](https://github.com/aws/aws-sdk-go), [iOS](https://github.com/aws/aws-sdk-ios), [Java](https://github.com/aws/aws-sdk-java), [JavaScript](https://github.com/aws/aws-sdk-js), [Python](https://github.com/boto/boto3), [Ruby](https://github.com/aws/aws-sdk-ruby) e [PHP](https://github.com/aws/aws-sdk-php) os mais usados. A AWS mantém [uma lista curta](https://aws.amazon.com/tools/#sdk), mas a [lista awesome-aws](https://github.com/donnemartin/awesome-aws#sdks-and-samples) é a mais abrangente e atual. Observe que o [suporte para C++](https://github.com/donnemartin/awesome-aws#c-sdk) é [ainda novo](https://aws.amazon.com/blogs/aws/introducing-the-aws-sdk-for-c/).
-	**Lógica de repetição:** Um aspecto importante a considerar sempre que usar SDKs é o tratamento de erros; sob uso intenso, pode-se esperar uma ampla variedade de falhas, desde erros de programação até limitação de recursos e falhas ou interrupções relacionadas à AWS. Os SDKs geralmente implementam [**exponential backoff**](https://docs.aws.amazon.com/general/latest/gr/api-retries.html) para lidar com isso, mas isso pode precisar ser entendido e ajustado ao longo do tempo para algumas aplicações. Por exemplo, muitas vezes é útil alertar sobre alguns códigos de erro e não sobre outros.
-	❗Não use APIs diretamente. Embora a documentação da AWS inclua muitos detalhes de API, é melhor usar os SDKs para sua linguagem preferida para acessar as APIs. Os SDKs são mais maduros, robustos e bem mantidos do que algo que você escreveria sozinho.

### Boto

-	Uma boa maneira de automatizar operações de forma personalizada é usando [**Boto3**](https://github.com/boto/boto3), também conhecido como [Amazon SDK for Python](http://aws.amazon.com/sdk-for-python/). [**Boto2**](https://github.com/boto/boto), a versão anterior desta biblioteca, tem sido amplamente usada por anos, mas agora há uma versão mais nova com suporte oficial da Amazon, então prefira o Boto3 para novos projetos.
-	O Boto3 contém uma variedade de APIs que operam em um nível alto ou baixo. Aqui está uma explicação de ambos:
	-	As APIs de baixo nível (APIs do Client) são mapeadas para APIs específicas de serviços da AWS Cloud, e todas as operações de serviço são suportadas por clientes. Os clientes são gerados a partir de um arquivo de definição de serviço JSON.
	-	A opção de alto nível, APIs de Resource, permite que você evite chamar a rede no nível baixo e, em vez disso, forneça uma maneira orientada a objetos para interagir com os serviços da AWS Cloud.
-	O Boto3 tem muitos [**recursos**](https://boto3.readthedocs.io/en/latest/guide/index.html#general-feature-guides) úteis, como *waiters*, que fornecem uma estrutura que permite que o código espere por mudanças ocorrerem na nuvem, por exemplo, quando você está criando uma instância EC2 e precisa esperar até que a instância esteja em execução para realizar outra tarefa.
-	Se você se encontrar escrevendo um script Bash com mais de um ou dois comandos CLI, provavelmente está fazendo isso de forma errada. Pare e considere escrever um script Boto em vez disso. Isso tem as vantagens de que você pode:
	-	Verificar os códigos de retorno facilmente para que o sucesso de cada etapa dependa do sucesso das etapas anteriores.
	-	Obter dados interessantes das respostas, como ids de instâncias ou nomes de DNS.
	-	Adicionar informações úteis do ambiente (por exemplo, marcar suas instâncias com revisões de git ou injetar o identificador da compilação mais recente no seu script de inicialização).

[Voltar ao topo :arrow_up:](#table-of-contents)

### Visibilidade Geral

-	🔹[**Taguear recursos**](http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/Using_Tags.html) é uma prática essencial, especialmente à medida que as organizações crescem, para entender melhor o uso dos seus recursos. Por exemplo, por meio de automação ou convenção, você pode adicionar tags:
	-	Para a organização ou desenvolvedor que “possui” aquele recurso
	-	Para o produto que o recurso suporta
	-	Para marcar ciclos de vida, como recursos temporários ou que devem ser desprovisionados no futuro
	-	Para distinguir infraestrutura crítica de produção (por exemplo, sistemas de serviço vs pipelines de backend)
	-	Para distinguir recursos com requisitos especiais de segurança ou conformidade
	-	Para (uma vez habilitado) [alocar custos](https://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/cost-alloc-tags.html). Observe que as tags de alocação de custos só se aplicam prospectivamente; você não pode aplicá-las retroativamente a itens já faturados.
	-	Por muitos anos, houve um limite notório de 10 tags por recurso, que não podia ser aumentado e causou dor significativa a muitas empresas. A partir de 2016, isso foi [aumentado](https://aws.amazon.com/blogs/security/now-organize-your-aws-resources-by-using-up-to-50-tags-per-resource/) para 50 tags por recurso.
	-	🔹Em 2017, a AWS introduziu a capacidade de [impor tagueamento](https://aws.amazon.com/blogs/aws/new-tag-ec2-instances-ebs-volumes-on-creation/) na criação de instâncias e volumes, depreciando partes de ferramentas de terceiros, como [Cloud Custodian](https://github.com/capitalone/cloud-custodian).
	-	🔸 As tags são sensíveis a maiúsculas e minúsculas; 'environment' e 'Environment' são duas tags diferentes. Automatizar a definição de tags é provavelmente a única opção sensata em escala significativa.
	-	🔸 Há um bug no console do ASG onde espaços após os nomes das tags são preservados. Então, se você digitar "Name " com um espaço no final, não terá o comportamento esperado. Isso provavelmente é verdadeiro em outros locais e SDKs também. Certifique-se de não adicionar espaços finais às chaves das tags, a menos que realmente queira. (A partir de julho de 2018)
	-	🔸 Quando os recursos são compartilhados pela organização, as tags não são compartilhadas com eles. Por exemplo, compartilhar o Transit Gateway ou AMIs mostrará as tags corretas na conta que criou esses recursos, mas não nas contas onde esses recursos foram compartilhados.
...
### AWS vs Configuração de Servidor

Este guia é sobre AWS, não sobre DevOps ou gerenciamento de configuração de servidor em geral. Mas antes de entrar em detalhes sobre a AWS, vale a pena notar que, além do gerenciamento de configuração para seus recursos da AWS, há o problema de longa data do gerenciamento de configuração para os próprios servidores.

[Voltar ao topo :arrow_up:](#table-of-contents)

### Filosofia

-	Os princípios do [**Twelve-Factor App**](http://12factor.net/) da Heroku listam algumas das melhores práticas gerais estabelecidas para implantar aplicações.
-	**Pets vs gado:** Trate os servidores [como gado, não como animais de estimação](https://www.engineyard.com/blog/pets-vs-cattle). Ou seja, projete sistemas para que a infraestrutura seja descartável. Não deve ser minimamente preocupante se um servidor for destruído inesperadamente.
-	O conceito de [**infraestrutura imutável**](http://radar.oreilly.com/2015/06/an-introduction-to-immutable-infrastructure.html) é uma extensão dessa ideia.
-	Minimize o estado da aplicação nas instâncias EC2. Em geral, as instâncias devem poder ser encerradas ou morrer inesperadamente com impacto mínimo. O estado que está na sua aplicação deve ser rapidamente movido para RDS, S3, DynamoDB, EFS ou outros armazenamentos de dados fora dessa instância. O EBS também é uma opção, embora geralmente não deva ser o volume inicializável, e o EBS exigirá remontagem manual ou automatizada.
